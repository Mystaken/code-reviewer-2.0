...
function validateArgs() {
  ...
  if (args.indexOf(params.env.name) > 0) {
    ...
    if (ret.environment !== "production" || ret.environment !== "development") {
      logger.fatal(
        "Environment can only be set to 'production' or 'development'."
      );
      ret.error = true;
      return ret;
    }
  ...
  if (ret.environment === "development") {
    if (args.indexOf(params.user_id.name) >= 0) {
      ...
      logger.debug(
        "No user_id inputted. Using default user_id: " + ret.user_id
      );
    }
...
function startApp() {
  ...
  if (opt.environment === "development") app.use(cors());
